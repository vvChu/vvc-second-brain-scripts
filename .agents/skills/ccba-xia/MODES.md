...
- `--copy-raw`: cấy ghép mã nguồn với số lượng thay đổi tối thiểu, đánh dấu tường minh các file chưa tuân thủ tiêu chuẩn Platform (gắn comment header `[XIA-COPY-RAW]`) và bắt buộc tạo follow-up Issue refactor. **Không được kết hợp với `--fast`.**
...
- `--fast`: rút gọn các pha nghiên cứu và phản biện. Pha 4 (Challenge) vẫn bắt buộc self-challenge ≥3 câu hỏi cốt lõi (không được bỏ qua hoàn toàn). Kế hoạch triển khai sẽ được gắn cảnh báo `[!WARNING]`. **Không được kết hợp với `--copy-raw`.**
