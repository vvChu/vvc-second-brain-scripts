...
Một **giai đoạn (Phase)** là một khối công việc độc lập trong một phiên làm việc của Agent — như giai đoạn phỏng vấn (`/ccba-grilling`), giai đoạn soạn spec và phân rã tác vụ (`/ccba-to-spec`), hoặc giai đoạn lập trình (`/ccba-implement`).
...
| **Clear Context (Làm sạch Context)** | Xóa sạch cửa sổ context, bắt đầu một phiên làm việc hoàn toàn mới. |
...
| **Compact Context (Nén Context)** | Nén lại toàn bộ context thành bản tóm tắt và khởi động phiên mới. |
...
- *Ví dụ*: Chuyển từ phỏng vấn (`/ccba-grilling`) sang bẻ spec và phân rã tác vụ (`/ccba-to-spec`) nên nằm trong **1 phiên duy nhất** để giữ nguyên lý do thiết kế ban đầu.
...
- **HÀNH ĐỘNG**: Thực hiện **Clear Context (Làm sạch Context)**. Đây là phương án tiết kiệm nhất: không mất thời gian và thu hồi lại 100% Context Budget.

> [!WARNING]
> Nếu lỡ xóa sạch (clear) một context *đang còn giá trị*, bạn sẽ mất hoàn toàn lý do (the **why**) đằng sau các quyết định kiến trúc cũ mà không thể phục hồi lại từ git diff.
...
- **HÀNH ĐỘNG**: Sử dụng **`/ccba-handoff`** khi:
  ...
  - Chuyển sang một **thư mục làm việc khác** (ví dụ: chuyển sang repo thử nghiệm trong `/ccba-implement`).
...
### 5. Nếu không thỏa mãn các điều kiện trên ➔ Nén Context (Compact Context)
- **HÀNH ĐỘNG**: Thực hiện **Compact Context (Nén Context)** kèm chỉ thị định hướng cụ thể (ví dụ: "Nén context và bắt đầu giai đoạn triển khai lập trình cho ticket #1").
- Nén context là **lựa chọn mặc định cuối cùng**, không phải là lựa chọn đầu tiên.
...
| **Secondary (Compact Context, `/ccba-handoff`)** | Có thể bị nén/mất mát | Thấp | Rất nhiều |
