...
- **Architectural Budgets & Zero-Slack Ratchets** (v8.15.13): See `AGENTS.md` §5. SSoT versioning qua `core.__version__`; trần tệp sản xuất mới $\le 350$ dòng (4 legacy ratchets); trần hàm AST $\le 50$ dòng (59 legacy ratchets); đồng bộ cây thư mục README sống và 0 rò rỉ đường dẫn máy (`test_architectural_budgets.py`, `test_codebase_cleanliness.py`).
