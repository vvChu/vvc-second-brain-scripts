...
- **v8.7 WebP Archive Compression (v8.7)**: `_archive_image()` trong `post_process.py` chuyển từ `shutil.copy()` sang nén Pillow WebP (RGB, 1536px max, quality=80). Giảm dung lượng lưu trữ ảnh `99 - Archive/` từ **2.19GB xuống ~170MB** (~92%). Fallback tự động về raw copy nếu Pillow gặp lỗi. Bổ sung unit test `test_archive_image_webp_compression` kiểm tra WebP output. 54/54 tests passed.
