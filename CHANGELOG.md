...
- **Zero-Slack Architectural Ratchets**: Thiết lập chốt chặn phân tích tĩnh trần tệp $\le 350$ dòng (29 tệp legacy) và trần hàm AST $\le 50$ dòng (83 tệp legacy) không khoảng đệm (`test_architectural_budgets.py`); kiên cố hóa regex linter Spoke Cleanliness chống rò rỉ đường dẫn máy (`test_codebase_cleanliness.py`).
- **Nâng Cấp Bộ Test Suite**: Bổ sung unit tests toàn diện (`test_wiki_health_package.py`, `test_weekly_synthesis.py`, `test_rag_package.py`, `test_codebase_cleanliness.py`, `test_architectural_budgets.py`), nâng tổng số test lên 571/571 passed (100% pass, 0 regressions).
