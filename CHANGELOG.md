...
- **Zero-Slack Architectural Ratchets & Graduation**: Thiết lập chốt chặn phân tích tĩnh trần tệp $\le 350$ dòng và trần hàm AST $\le 50$ dòng không khoảng đệm; phân rã và tinh gọn `services/command/hero_image.py` về 345 dòng, `services/excalidraw_worker.py` về 298 dòng, `services/youtube/transcript.py` về 349 dòng, `knowledge_graph/parse_graph.py` về 330 dòng, `pipeline/book_assets.py` về 345 dòng, `services/mermaid_worker.py` về 322 dòng, `check_hub_import_depth.py` về 338 dòng, `tools/heal_video_frames.py` về 347 dòng, `tools/diagram_template_builder.py` về 303 dòng, `book_ingest.py` về 345 dòng, `epub_convert.py` về 340 dòng, `pipeline/ocr.py` về 338 dòng, và `pipeline/map_reduce.py` về 327 dòng ($\le 350$ dòng), khử 100% hàm > 50 dòng, chính thức tốt nghiệp khỏi danh sách legacy ratchets, siết `LEGACY_LINE_RATCHET` xuống 16 tệp và `LEGACY_FUNC_OVER_50_BUDGET` xuống 70 tệp (`test_architectural_budgets.py`); kiên cố hóa regex linter Spoke Cleanliness chống rò rỉ đường dẫn máy (`test_codebase_cleanliness.py`).
