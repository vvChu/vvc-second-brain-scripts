...
- **Zero-Slack Architectural Ratchets & Graduation**: Thiết lập chốt chặn phân tích tĩnh trần tệp $\le 350$ dòng và trần hàm AST $\le 50$ dòng không khoảng đệm; phân rã và tinh gọn `services/command/hero_image.py` về 345 dòng, `services/excalidraw_worker.py` về 298 dòng, `services/youtube/transcript.py` về 349 dòng, `knowledge_graph/parse_graph.py` về 330 dòng, `pipeline/book_assets.py` về 345 dòng, `services/mermaid_worker.py` về 322 dòng, `check_hub_import_depth.py` về 338 dòng, `tools/heal_video_frames.py` về 347 dòng, `tools/diagram_template_builder.py` về 303 dòng, `book_ingest.py` về 345 dòng, `epub_convert.py` về 340 dòng, `pipeline/ocr.py` về 338 dòng, `pipeline/map_reduce.py` về 327 dòng, `pipeline/ground_truth.py` về 342 dòng, `services/podcast.py` về 302 dòng, `knowledge_graph/verify_graph.py` về 334 dòng, `pipeline/post_process.py` về 299 dòng (tách `pipeline/image_archiver.py` 155 dòng), `pipeline/semantic_merger.py` về 303 dòng (tách `pipeline/semantic_fallback.py` 208 dòng), `services/moc_mermaid.py` về 232 dòng (tách `services/moc_source_diagram.py` 195 dòng), `services/article_images.py` về 271 dòng (tách module `services/article_image_parser.py` 109 dòng), `services/command/coordinator.py` về 313 dòng (tách module `services/command/query_context.py` 257 dòng), `tools/batch_ingest_books.py` về 112 dòng (tách dữ liệu fixtures sang `tools/data/` JSON), `pipeline/image_processor.py` về 236 dòng (tách module `pipeline/batch_processor.py` 345 dòng), `core/vector_store.py` về 331 dòng (tách `core/vector_io.py` 140 dòng và `core/vector_sync.py` 217 dòng), `services/brain_dump/concept_synthesis.py` về 202 dòng (tách `services/brain_dump/transcript_writer.py` 297 dòng và `services/brain_dump/cross_links.py` 122 dòng), `wiki_maintain.py` về 250 dòng (tách `services/moc_builder.py` 237 dòng và `services/master_index.py` 248 dòng), `daemon.py` về 331 dòng (tách `services/daemon_queue.py` 174 dòng, mở rộng `core/daemon_utils.py` 178 dòng), `services/diagram_base.py` về 232 dòng (tách `services/diagram_geometry.py` 168 dòng, `services/diagram_templates.py` 159 dòng, `services/diagram_fallback.py` 222 dòng), và `services/youtube/visual_extractor.py` về 303 dòng (tách `services/youtube/storyboard_sampler.py` 344 dòng, `services/youtube/visual_judge.py` 262 dòng, `services/youtube/stage2_extractor.py` 273 dòng) ($\le 350$ dòng), khử 100% hàm > 50 dòng, chính thức tốt nghiệp hoàn toàn khỏi danh sách legacy line ratchets (**`LEGACY_LINE_RATCHET` = 0 tệp** — 100% tệp sản xuất tuân thủ trần $\le 350$ dòng) và siết `LEGACY_FUNC_OVER_50_BUDGET` xuống 55 tệp (`test_architectural_budgets.py`); kiên cố hóa regex linter Spoke Cleanliness chống rò rỉ đường dẫn máy (`test_codebase_cleanliness.py`).
...
- **Zero Function Length Violations (100% Adherence to Global Rule 5)**: Hoàn tất đợt refactor toàn diện trên toàn bộ 55 tệp legacy có hàm $> 50$ dòng; phân rã 69 hàm dài thành các modular helpers súc tích và có type-hints; bảo đảm 100% tệp sản xuất $\le 350$ dòng; chính thức đưa ngân sách hàm dài về số 0 tuyệt đối (**`LEGACY_FUNC_OVER_50_BUDGET = {}`** — 0 tệp vi phạm trên toàn bộ codebase `scripts/`).
...
- **Đúc Kết Quy Tắc Quản Trị Vận Hành & Thể Chế Hóa Zero-Slack (Operational Governance & Zero-Slack Ratchets)**: Thể chế hóa trạng thái Zero-Slack Ratchets toàn diện vĩnh viễn (0 legacy line ratchets, 0 legacy function ratchets); bổ sung Proactive Decomposition Invariant (ngưỡng cảnh báo sớm $\ge 40$ dòng AST chống Locality Bias) và Zero-Tradeoff Invariant (cấm gộp hàm né trần tệp, bắt buộc bóc tách deep module package); thể chế hóa vùng đệm tệp tạm thời `.md/scratch/` giải quyết xung đột với `.gitignore`; và đồng bộ 100% tài liệu context files theo Sync Manifest.
