...
- **Zero-Slack Architectural Ratchets & Graduation**: Thiết lập chốt chặn phân tích tĩnh trần tệp $\le 350$ dòng (27 tệp legacy) và trần hàm AST $\le 50$ dòng (81 tệp legacy) không khoảng đệm; phân rã và tinh gọn `services/command/hero_image.py` về 345 dòng và `services/excalidraw_worker.py` về 298 dòng ($\le 350$ dòng), khử 100% hàm > 50 dòng ở cả 2 tệp, chính thức tốt nghiệp khỏi danh sách legacy ratchets (`test_architectural_budgets.py`); kiên cố hóa regex linter Spoke Cleanliness chống rò rỉ đường dẫn máy (`test_codebase_cleanliness.py`).
