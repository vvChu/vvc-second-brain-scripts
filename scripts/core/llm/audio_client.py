...
def _build_gateway_audio_payload(
    model: str, language: str | None
) -> tuple[dict[str, str], dict[str, str]]:
    """Build HTTP headers and form payload for audio transcription request."""
    ...
    if language and language.lower() not in ("auto", "none", ""):
        ...
        if language == "en":
            data["prompt"] = (
                "Below is the detailed content in English, with full punctuation and capitalization:"
            )
        else:
            data["prompt"] = (
                "Dưới đây là nội dung chi tiết bằng tiếng Việt, có đầy đủ dấu câu:"
            )
    return headers, data


def _parse_audio_response_json(response) -> tuple[list, str]:
    """Extract segment items and raw transcription string from gateway response."""
    segments = []
    raw_text = ""
    try:
        ...
        if (raw_result.startswith("{") and raw_result.endswith("}")) or (
...
def _format_audio_segments(segments: list, raw_text: str) -> str:
    """Format segments into 30s timestamped chunks or paragraph fallback."""
    if segments:
        formatted_lines = []
        curr_start = None
        curr_texts = []
...
def call_gateway_audio(
    ...
    *,
    ...
    try:
        ...
        headers, data = _build_gateway_audio_payload(model, language)
        ...
        with open(audio_path, "rb") as audio_file:
            ...
            response = requests.post(
                url, headers=headers, data=data, files=files, timeout=req_timeout
            )
            response.raise_for_status()
            segments, raw_text = _parse_audio_response_json(response)
            return _format_audio_segments(segments, raw_text)
