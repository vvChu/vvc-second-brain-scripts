...
class VaultLinter:
    ...
    def _evaluate_concept_metrics(
        ...
        for link in c.get("_links", []):
            ...
            if normalized not in self.existing_stems:
                report["broken_links"].append({"from": stem, "to": link})
    ...
    def lint(self) -> LintReport:
        ...
        log("lint", f"Health check: {report['total_concepts']} concepts scanned")
        return report
...
def heal_broken_links(report: LintReport | None = None, max_heal_limit: int = 15) -> int:
    """Auto-create concept stubs for broken wiki-links (Facade pattern)."""
    if report is None:
        report = lint_vault()
    return LinkHealer().heal(report, max_heal_limit)
