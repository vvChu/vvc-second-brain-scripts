...
from core.llm import _call_gemini_cli
...
print('Starting Gemini CLI Model Evaluation...')
for m in models_to_test:
    print(f'\n--- Testing {m} ---')
    start = time.time()
    try:
        res = _call_gemini_cli(test_prompt, model=m, timeout=60)
        dur = time.time() - start
        if res:
            print(f'SUCCESS ({dur:.1f}s)\nOutput: {res}')
        else:
