...
5. **Markdown Table Invariants (v8.15.2)**: Bắt buộc escape pipe trong wikilinks (`[[slug\|alias]]`), non-breaking arrow (`↳&nbsp;Text`), subtitle baseline width stabilization `*(...)*`, và phân tầng 2 nhịp ($\le 40$ ký tự/dòng).
   - **Table Title Pipe Escaping**: Khi render liên kết bảng từ tiêu đề concept có thể chứa ký tự pipe (`title`), BẮT BUỘC xử lý thoát pipe kép bảo đảm tính lũy thừa (idempotency): `clean_title = raw_title.replace(r"\|", "|").replace("|", r"\|")` nhằm triệt tiêu nguy cơ tách thêm cột làm vỡ layout bảng.
...
8. **Clean Wikilink & Zero-Code-Pill Invariant (v8.15.7 / v8.15.11)**: Cấm tuyệt đối bọc backticks quanh wikilinks hoặc embed tags trên mọi bề mặt Markdown (thân bài, bảng biểu, callouts, footnotes, danh mục tham chiếu). Backtick biến liên kết tương tác thành code pill xám và làm gãy đồ thị Graph View trong Obsidian. Khi cần viết ví dụ cú pháp hoặc placeholder cho liên kết (`[[...]]` hoặc `![[...]]`), BẮT BUỘC đặt trong fenced code block chuẩn (ngôn ngữ `text` hoặc `markdown`); tuyệt đối cấm dùng inline backticks nhằm ngăn linter báo lỗi code-pill và triệt tiêu nguy cơ bị regex sanitizer bóc tách làm gãy đồ thị. Trong bảng Markdown, bắt buộc escape pipe (`[[slug\|alias]]`) nhưng giữ nguyên liên kết trần; số thứ tự trích dẫn trong bảng phải đồng bộ 1-1 với danh mục tham chiếu cuối bài.
...
- **Deterministic Technical Debt Prevention & Zero-Slack Ratchets (v8.15.13)**: Để chặn đứng nguy cơ tích tụ nợ kỹ thuật, phình to file, và thoái hóa mã nguồn do thiên kiến cục bộ (Locality Bias):
  1. **SSoT Versioning**: [`scripts/core/__version__.py`](scripts/core/__version__.py) là nguồn chân lý duy nhất cho phiên bản. Cấm tuyệt đối hardcode chuỗi version trong daemon logging hoặc mã nguồn.
  2. **Zero-Slack Line Budget (≤ 350 lines)**: 100% tệp Python mã nguồn sản xuất (ngoại trừ tests và whitelist `core/prompts/*.py` chứa prompt hiến pháp) bắt buộc $\le 350$ dòng. Codebase duy trì 0 legacy line ratchets (`LEGACY_LINE_RATCHET = {}`).
  3. **Zero-Slack AST Function Length Budget (≤ 50 lines)**: Cưỡng chế cơ học Global Rule 5 qua AST analysis. 100% hàm sản xuất bắt buộc $\le 50$ dòng. Codebase duy trì 0 legacy function ratchets (`LEGACY_FUNC_OVER_50_BUDGET = {}`).
  4. **Proactive Decomposition & Zero-Tradeoff Invariant**:
     - *Chống Locality Bias*: Khi bổ sung logic, sửa lỗi hoặc xử lý edge case, nếu hàm chạm ngưỡng $\ge 40$ dòng, CẤM chèn thêm mã inline; BẮT BUỘC chủ động phân rã thành các helper functions chuyên biệt (KISS).
     - *Chống Budget Trade-off*: CẤM gộp nhiều bước xử lý vào chung một hàm đơn khối để né trần tệp 350 dòng. Nếu việc tách hàm khiến tệp vượt 350 dòng, giải pháp BẮT BUỘC duy nhất là tái cấu trúc tệp thành deep module package (`scripts/<layer>/<module>/`).
  5. **Live Directory Tree Parity**: 100% các deep module packages trong `scripts/core/` và `scripts/services/` bắt buộc phải được ghi nhận đầy đủ trong `scripts/README.md` (kiểm tra tự động qua `test_readme_directory_tree_parity`).
  6. **Ratchet Integrity & Zero-Slack Preservation**: Mọi pull request hoặc commit mới vi phạm trần dòng hoặc trần hàm sẽ làm gãy `pytest` ngay lập tức. Tuyệt đối CẤM tự ý mở thêm mục ngân sách ratchet trong `scripts/tests/test_architectural_budgets.py` để lách lỗi kiểm thử.
